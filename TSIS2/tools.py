# tools.py

import pygame
import collections

# запуск pygame
pygame.init()

# размеры окна
SCREEN_W = 950
SCREEN_H = 680
TOOLBAR_H = 64   # верхняя панель

# основные цвета
WHITE = (255, 255, 255)
BLACK = (0, 0, 0)
GRAY = (90, 90, 90)
DARK = (30, 30, 40)
BLUE = (70, 130, 220)
GREEN = (60, 180, 100)

# палитра цветов
PALETTE = [
    (0,0,0),(255,255,255),(255,0,0),(0,255,0),(0,0,255),
    (255,255,0),(255,0,255),(0,255,255),(128,128,128)
]

# названия инструментов
PEN = "pen"
LINE = "line"
RECT = "rect"
CIRCLE = "circle"
ERASER = "eraser"
FILL = "fill"
TEXT = "text"
SQUARE = "square"
RTRI = "rtri"
ETRI = "etri"
RHOMB = "rhomb"

# создаем окно
screen = pygame.display.set_mode((SCREEN_W, SCREEN_H))
pygame.display.set_caption("Paint")

# fps
clock = pygame.time.Clock()

# шрифт
font = pygame.font.SysFont("Arial", 16)

# холст
canvas = pygame.Surface((SCREEN_W, SCREEN_H - TOOLBAR_H))
canvas.fill(WHITE)


# класс кнопки
class Button:
    def __init__(self, x, y, w, h, text, tool):
        self.rect = pygame.Rect(x, y, w, h)
        self.text = text
        self.tool = tool

    # рисуем кнопку
    def draw(self, win, active):
        color = BLUE if active == self.tool else DARK
        pygame.draw.rect(win, color, self.rect)
        pygame.draw.rect(win, GRAY, self.rect, 1)

        txt = font.render(self.text, True, WHITE)
        win.blit(txt, (self.rect.x + 5, self.rect.y + 3))

    # нажали или нет
    def click(self, pos):
        return self.rect.collidepoint(pos)


# перевод координат вниз панели
def to_canvas(x, y):
    return x, y - TOOLBAR_H


# заливка области
def flood_fill(surface, pos, new_color):

    x, y = pos
    w, h = surface.get_size()

    if not (0 <= x < w and 0 <= y < h):
        return

    old = surface.get_at((x, y))[:3]

    if old == new_color:
        return

    q = collections.deque()
    q.append((x, y))
    used = set()

    while q:
        cx, cy = q.popleft()

        if (cx, cy) in used:
            continue

        used.add((cx, cy))

        if surface.get_at((cx, cy))[:3] == old:
            surface.set_at((cx, cy), new_color)

            for dx, dy in [(1,0),(-1,0),(0,1),(0,-1)]:
                nx = cx + dx
                ny = cy + dy

                if 0 <= nx < w and 0 <= ny < h:
                    q.append((nx, ny))


# рисуем верхнюю панель
def draw_toolbar(win, buttons, tool, color, size, palette_rects):

    pygame.draw.rect(win, DARK, (0, 0, SCREEN_W, TOOLBAR_H))

    # кнопки инструментов
    for b in buttons:
        b.draw(win, tool)

    # текущий цвет
    pygame.draw.rect(win, color, (430, 10, 20, 40))

    # размер кисти
    txt = font.render("Size: " + str(size), True, WHITE)
    win.blit(txt, (460, 20))

    # палитра
    for i in range(len(palette_rects)):
        pygame.draw.rect(win, PALETTE[i], palette_rects[i])
        pygame.draw.rect(win, BLACK, palette_rects[i], 1)